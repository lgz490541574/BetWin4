...
namespace BW.Games.API
{
    ...
    /// IM体育和电竞牛共用的基类
    /// </summary>
    public abstract class IM : IGameBase
    {
        #region ========= 配置参数 ===========
        ...
        protected IM(string queryString) : base(queryString)
        {
        }
        ...
        public override RegisterResult Register(RegisterRequest register)
        {
            ...
            {
                { "MerchantCode", this.MerchantCode },
                { "PlayerId", this.GetUserName(register) },
                { "Currency", "CNY" },
                { "Password", password },
                { "Country", "CN" },
                { "Sex", "M" },
        ...
        public override TransferResult Recharge(TransferRequest transfer)
        {
            ...
            return new TransferResult(resultType);
        }

        public override BalanceResult Balance(BalanceRequest balance)
        {
            APIResultType resultType = this.POST("/Player/GetBalance", new()
            {
                { "MerchantCode", this.MerchantCode },
                { "PlayerId", balance.UserName },
                { "ProductWallet", this.ProductWallet }

            }, out object info);

            if (resultType != APIResultType.Success) throw new APIResultException(resultType);
            return new BalanceResult(balance.UserName, ((JObject)info)["Balance"].Value<decimal>());
        }
